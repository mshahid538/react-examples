...
function Counter() {  
    ...
    const [student, setStudent] = React.useState({name: 'shahajhan', age: 44, gender: 'Male'});
    ...
    return(
        <>
            ...
            <button onClick={ () => { setStudent({...student, age: 55}) }}>change age</button>
            { isShowBooks && (
                <>
                    <h2>Books <span>Total books price: {TotalBooksAmount}</span></h2>
                    <ul>
                        { books.map( book => {
                        return <li style={{ height: '120px', width: '100%', border: '1px solid #ccc', marginBottom: '20px'}}>
